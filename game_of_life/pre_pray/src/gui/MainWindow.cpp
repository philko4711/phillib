...
void MainWindow::updateMapObjects(
  ...
  for (auto &iter : mapObjects) {
    ...
    if (iter->type() == IObjectMap::Type::FOOD) 
    {
      qt::QRectFilled rectFood(QRect(iter->pos() - QPoint(5, 5), QSize(10, 10)), color);   //Todo: this is for testing. Make size variable or use a const factor
      rectsFilled.push_back(rectFood);
    }
    else if (iter->type() == IObjectMap::Type::PREY) // || todo: Predator
    {
